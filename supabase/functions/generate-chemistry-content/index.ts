...
serve(async (req) => {
  ...
  try {
    ...
    const userPrompt = type === 'scenario'
      ? 'Generate a relationship scenario with 3 response options. Respond with ONLY valid JSON: {"title": string, "description": string, "options": string[]}. Make it engaging and realistic.'
      : 'Generate a romantic experiment for couples. Respond with ONLY valid JSON: {"title": string, "description": string, "duration": string, "materials": string[]}. Make it fun and meaningful.'
    ...
    console.error('generate-chemistry-content error:', error)
